...
if (isset($_POST['adminSignup'])) {
    ...
    if (!$name_err && !$panelNum_err && !$email_err && !$pass_err && !$cpass_err) {
        ...
        $sql = "INSERT INTO admin(name, staffnum, email, pass, salt) VALUES ('$name', '$staffNum', '$email', '$hashed_password', '$salt')";
...
<body class="login-pic fade-in">
    ...
    <div class="row">
        ...
        <div class="box">
        <button type="submit" class="btn btn2 btn-primary" onclick="window.location.href='./adminManage.php'">Back</button>
            <h4 class="text-center mb-3">Generate Admin Account:</h4>

            <h4 class="text-center mt-3 mb-3" id="label1">Please fill all the fields below</h4>
            <!-- admin Form -->
          <form method="post" action="generateAdminAccount.php" id ="panelSignupForm">
          ...
          <div id="panelContent">
            ...
            <button type="submit" id="adminSignup" name="adminSignup" class="btn btn-success btn-block login-btn btn-custom-shadow custom-btn-signup">REGISTER</button>
          </div>
          </form>
