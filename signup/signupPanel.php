...
// initialize variable
$name_err=$panelNum_err=$email_err=$pass_err=$cpass_err="";
$name=$panelNum=$email=$pass=$cpass= "";
...
if (isset($_POST['panelSignup'])) {
    ...
    $panelNum = mysqli_real_escape_string($conn, $_POST['panelNum']);
    ...
    if(preg_match("/^[0-9]+$/", $panelNum)){
      if(strlen($panelNum) < 4){
        $panelNum_err = "Please enter valid staff number";
      }
    }else{
      $panelNum_err = "No special character is allowed";
    }
    // email verification
    ...
    // saved data into database if no error
    if (!$name_err && !$panelNum_err && !$email_err && !$pass_err && !$cpass_err) {
        
        $hashed_password = generate_hashed_password($pass, $salt);

        
        $sql = "INSERT INTO panel(name, panelnum, email, pass, verify, salt) VALUES ('$name', '$panelNum', '$email', '$hashed_password', 0, '$salt')";
...
<body class="login-pic fade-in">
    ...
    <div class="row">
        ...
        <div class="box">
            ...
            <!-- Panel Form -->
          <form method="post" action="signupPanel.php" id ="panelSignupForm">
          ...
          <div id="panelContent">
            ...
            <div class="form-floating mb-3 ms-5 me-5 custom-textbox">
                <input required type="text" class="form-control" id="panelNum" name="panelNum" placeholder="Enter staff number" value="<?php echo isset($_POST['panelNum']) ? htmlspecialchars($_POST['panelNum']) : ''; ?>">
                <label for="panelNum">Panel Number</label>
            </div>
            <!-- error message --><p><span class="error-msg"><?php echo $panelNum_err ?></span></p>
            <div class="form-floating mb-3 ms-5 me-5 custom-textbox">
            ...
            <button type="submit" id="panelSignup" name="panelSignup" class="btn btn-success btn-block login-btn btn-custom-shadow custom-btn-signup">REGISTER</button>
          </div>
          </form>
  ...
  <script>
    ...
    // changing location when pressing button
    ...
    role4.checked = true; // let it stay checked
...
// pop up success message upon successfully saved data
