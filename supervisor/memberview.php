...
if($infoUser[0]['grpid'] != ""){
    ...
    if(isset($_POST['submit'])){
        ...
        $trait1mark = $trait1*(7/15); // contribute 3.5%
        $trait2mark = $trait2*(2/15);
        $trait3mark = $trait3*(2/15);
        $trait4mark = $trait4*(2/15);
        $trait5mark= $trait5*(2/15);

        $totalMark = $trait1mark  + $trait2mark  + $trait3mark  + $trait4mark + $trait5mark;
        ...
        if(!empty($memberid) && !empty($svid)){
            ...
            if(isset($peerRecord)){
                ...
                $sql = "UPDATE peersv SET trait1='$trait1', trait2='$trait2', trait3='$trait3', trait4='$trait4', trait5='$trait5', timeSubmit='$submitTime', comment='$comment' WHERE svid='$svid' AND studentid='$memberid' AND grpid='$grpid'";
                ...
                $sql = "INSERT INTO peersv (svid, studentid, trait1, trait2, trait3, trait4, trait5, comment, timeSubmit, grpid) VALUES ('$svid', '$memberid', '$trait1', '$trait2', '$trait3', '$trait4', '$trait5', '$comment', '$submitTime', '$grpid')";
        ...
        <div class="main">
            ...
            <form class="profile-form" method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
                ...
                <div class="row g-3 align-items-center">
                    <div class="col-auto">
                        <label for="leader" class="col-form-label">Member : <?php echo isset($memberinfo[0]['name'])?strtoupper(" ".$memberinfo[0]['name']):'';?></label>
                    </div>
                </div>
                <!-- ques1 -->
                <div class="row g-3 align-items-center py-2">
                    <div class="text-justify mb-3">
                        <div>
                        <strong>1. Teamwork Skill </strong>
                        </div>
                        <div class="form-check form-check-inline">
                            <input  required class="form-check-input" type="radio" name="trait1" id="trait1" value="0" <?php echo (isset($peerRecord[0]['trait1']) && $peerRecord[0]['trait1'] == 0)?"checked":'';?>/>
                        ...
                        <div class="form-check form-check-inline">
                            <input required  class="form-check-input" type="radio" name="trait1" id="trait1" value="3" <?php echo (isset($peerRecord[0]['trait1']) && $peerRecord[0]['trait1'] == 3)?"checked":'';?>/>
