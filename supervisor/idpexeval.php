...
if(isset($_GET['id'])){
    ...
    $sql = "SELECT * FROM idpexeval WHERE grpid='$grpid' AND evalType='supervisor' AND evaluatorid='$svid'";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0){
...
if(isset($_POST['submit'])){
    ...
    if (mysqli_num_rows($result) > 0){
        ...
        if (mysqli_query($conn, $sql)) {
            $sql = "UPDATE grpmark SET idpexMarkSV='$finalMark' WHERE grpid = '$grpid'";
            if (mysqli_query($conn, $sql)) {
        ...
        $sql = "INSERT INTO idpexeval (evaluatorid, evalType, grpid, trait1, trait2, trait3, trait4, trait5, trait6, trait7, trait8, comment, submitTime) VALUES ('$svid', 'supervisor', '$grpid', '$trait1', '$trait2', '$trait3', '$trait4', '$trait5', '$trait6', '$trait7', '$trait8', '$comment', '$submitTime')";
        if (mysqli_query($conn, $sql)) {
            ...
            if (mysqli_num_rows($result) > 0){
                ...
                $sql = "INSERT INTO grpmark (grpid, idpexMarkSV) VALUES ('$grpid', '$finalMark')";
                if (mysqli_query($conn, $sql)) {
                    echo "<script>alert('Successfully evaluate');</script>";
                    echo "<script>window.location.href = 'idpexevalHome.php';</script>";
                }else {
            ...
            <form class="profile-form" method="post" action="">
                ...
                <div class="row g-3 align-items-center py-2">
                    ...
                    <h5 class="text-center py-3">
                            <strong>
                                You are now evaluating Group <?php echo isset($infoGrp[0]['grpnum'])?htmlspecialchars($infoGrp[0]['grpnum']):''; ?>
                            </strong>   
                    </h5>
                <div class="row g-3 align-items-center py-2">
