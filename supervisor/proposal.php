<?php
    ...
    if($infoUser[0]['grpid'] != ""){
        ...
        $sqlFile = "SELECT * FROM idpfile WHERE grpid = '$grpid' AND type = 'proposal'";
        ...
        if(mysqli_num_rows($resultFile) > 0){
            ...
            // load the evaluation data information 
            $sql = "SELECT submitTime FROM proposalmark WHERE grpid='$grpid'";
            $result = mysqli_query($conn, $sql);
            if(mysqli_num_rows($result) > 0){
                $info = mysqli_fetch_all($result, MYSQLI_ASSOC);
                $evalTime = $info[0]['submitTime'];
            }

            // load the evaluation mark
            $sql = "SELECT proposalMark FROM grpmark WHERE grpid='$grpid'";
            $result = mysqli_query($conn, $sql);
            if(mysqli_num_rows($result) > 0){
                $info = mysqli_fetch_all($result, MYSQLI_ASSOC);
                $markTemp = $info[0]['proposalMark'];
                $mark = ($markTemp / 10)*100;
            }
        }
    ...
    $sqlDuetime = "SELECT duedate.dueDateTime FROM duedate INNER JOIN context ON duedate.contid = context.contid WHERE context = 'proposal'";
    ...
    if (isset($_POST['approve'])) {
        ...
        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Project Proposal Approved, Student no longer able to edit');</script>";
        } else {
            echo 'Query error: ' . mysqli_error($conn);
        }
        //file information to ensure it fill the textbox
    ...
    if (isset($_POST['reject'])) {
        ...
        if (mysqli_query($conn, $sql)) {
            echo "<script>alert('Project Proposal Rejected, Student able to make changes');</script>";
        } else {
            echo 'Query error: ' . mysqli_error($conn);
        }
        //file information to ensure it fill the textbox
...
<head>
    ...
    @media (max-width: 768px){
        ...
        <div class="main">
            ...
            <div class="container">
                <div class="proposal-form">
                    <div class="title-container">
                        <h1 class="text-center">Project Proposal</h1>
                    </div>
                        ...
                        if (isset($infoFile)) {
                            foreach($infoFile as $row){
                                ...
                                <tr>
                                    ...
                                    <td><?php echo isset($evalTime)?htmlspecialchars($evalTime):''; ?></td>
                                    <td><?php echo isset($mark)?htmlspecialchars($mark):''; ?>%</td>
                                    <td>
                                        ...
                                        <a href="./proposaleval.php" class="btn btn-sm btn-custom-action" style="background-color: #2FAB74;"><i class="fas fa-edit pe-1"></i><span class="custom-display">Evaluate</span>
                                    </td>
            ...
            <div class="container">
                <form class="proposal-form" action="proposal.php" method="post">
            ...
            <div class="container mt-5">
                <a id="download-pdf" class="btn btn-primary btn-custom-shadow" href="../fileuploaded/rubrics/rubric_proposal.pdf" download="rubric_proposal.pdf" style="background-color: #4CE833; border: none; color: black;"><i class="fa-solid fa-file-arrow-down pe-1"></i><span class="custom-display">Download</span> Rubrics</a>
            </div>
            <div class="container">
