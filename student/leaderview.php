...
if($infoUser[0]['grpid'] != ""){
    ...
    if(isset($_POST['submit'])){
        ...
        $leadtrait = mysqli_real_escape_string($conn,$_POST['trait3']);
        ...
        $totalMark = $trait1mark +$leadtraitmark +$trait4mark +$trait5mark + $trait6mark + $trait7mark;
        ...
        if(!empty($trait1) && !empty($leadtrait) && !empty($trait4) && !empty($trait5) && !empty($trait6) && !empty($trait7) && !empty($memberid) && !empty($studentid)){
            
            if(isset($peerRecord)){
                // if set means need update only!
                $sql = "UPDATE peerstudent SET trait1='$trait1', leadtrait='$leadtrait', trait4='$trait4', trait5='$trait5', trait6='$trait6', trait7='$trait7', timeSubmit='$submitTime', comment='$comment', mark='$finalMark' WHERE grader='$studentid' AND member='$memberid' AND grpid='$grpid'";
                ...
                $sql = "INSERT INTO peerstudent (grader, member, trait1, trait4, trait5, trait6, trait7, leadtrait, comment, timeSubmit, grpid, mark) VALUES ('$studentid', '$memberid', '$trait1', '$trait4', '$trait5', '$trait6', '$trait7', '$leadtrait', '$comment', '$submitTime', '$grpid', '$finalMark')";
    ...
    @media(max-width: 768px){
        ...
        <div class="main">
            ...
            <form class="profile-form" method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
                ...
                <!-- remove trait2 because it is only for members -->
                <!-- ques3 -->
                <div class="row g-3 align-items-center pt-2">
                    <h5 class="text-justify">
                        <strong>2. Leadership Quality:</strong>
                    </h5>
                </div>
                <div class="row g-3 align-items-center pt-0">
                    <h6 class="text-justify">
                    Takes the initiative, makes suggestions and provides focus. Creative. Energitic. 
                    Brings energy and excitement to the team. Has a "can do" attitude. Spearks creativity in others. 
                    Monitor the effectiveness of the group. 
                    </h6>
                </div>
                <div class="text-center mb-3">
                    <div class="form-check form-check-inline">
                    <input required class="form-check-input" type="radio" name="trait3" id="trait3" value="5" <?php echo (isset($peerRecord[0]['leadtrait']) && $peerRecord[0]['leadtrait'] == 5) ? "checked" : ''; ?>/>
                    <label class="form-check-label" for="trait3">5</label>
                    </div>
                    <div class="form-check form-check-inline">
                    <input required class="form-check-input" type="radio" name="trait3" id="trait3" value="4" <?php echo (isset($peerRecord[0]['leadtrait']) && $peerRecord[0]['leadtrait'] == 4) ? "checked" : ''; ?>/>
                    <label class="form-check-label" for="trait3">4</label>
                    </div>
                    <div class="form-check form-check-inline">
                    <input required class="form-check-input" type="radio" name="trait3" id="trait3" value="3" <?php if(!isset($peerRecord[0]['leadtrait'])){echo "checked";} ?><?php echo (isset($peerRecord[0]['leadtrait']) && $peerRecord[0]['leadtrait'] == 3) ? "checked" : ''; ?>/>
                    <label class="form-check-label" for="trait3">3</label>
                    </div>
                    <div class="form-check form-check-inline">
                    <input required class="form-check-input" type="radio" name="trait3" id="trait3" value="2" <?php echo (isset($peerRecord[0]['leadtrait']) && $peerRecord[0]['leadtrait'] == 2) ? "checked" : ''; ?>/>
                    <label class="form-check-label" for="trait3">2</label>
                    </div>
                    <div class="form-check form-check-inline">
                    <input required class="form-check-input" type="radio" name="trait3" id="trait3" value="1" <?php echo (isset($peerRecord[0]['leadtrait']) && $peerRecord[0]['leadtrait'] == 1) ? "checked" : ''; ?>/>
                    <label class="form-check-label" for="trait3">1</label>
                    </div>
                </div>
                <!-- ques4 -->
                <div class="row g-3 align-items-center pt-2">
                ...
                <div class="text-center mb-3">
                    ...
                    <label class="form-check-label" for="inlinetrait5Radio1">5</label>
                    </div>
                    <div class="form-check form-check-inline">
